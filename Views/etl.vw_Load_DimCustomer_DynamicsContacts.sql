...
/*AddressPrimary*/
      , ISNULL(c.address1_line1,'') + ' ' + ISNULL(c.address1_line2,'') AS [AddressPrimaryStreet]
...
/*Phone*/
      , CAST(c.telephone1 AS NVARCHAR(25)) AS [PhonePrimary]
      , CAST(c.telephone2 AS NVARCHAR(25)) AS [PhoneHome]
      , CAST(c.telephone3 AS NVARCHAR(25)) AS [PhoneCell]
      , CAST(c.telephone1 AS NVARCHAR(25)) AS [PhoneBusiness]
      , NULL AS [PhoneFax]
      , NULL AS [PhoneOther]
...
/*Source Created and Updated*/
	  ...
	  , NULL AS [AccountId]
	  , CASE WHEN c.new_ssbcrmsystemssidwinnersourcesystem = 'TM'
				AND ISNULL(c.new_ssb_crmsystemssidwinner,'') != ''
			THEN CONCAT(c.new_ssbcrmsystemssidwinnersourcesystem, '-',c.new_ssb_crmsystemssidwinner)
		ELSE NULL END AS [Customer_Matchkey]
      , 0 IsBusiness

--		select top 100 *
		FROM predators_Reporting.prodcopy.Contact AS c WITH ( NOLOCK )
		WHERE   1 = 1
		AND ( (createdon > DATEADD(DAY,-3,GETDATE()) OR modifiedon > DATEADD(DAY,-3,GETDATE()) OR c.CopyLoadDate > DATEADD(DAY,-3,GETDATE()))
		OR c.statecode = 1)
