...
CREATE VIEW [etl].[crm_contact_migration_20161005] AS
SELECT CAST(id.ContactId AS UNIQUEIDENTIFIER) AS 'ContactID',
       ...
       cab.PostOfficeBox ,
       cab.PostalCode
FROM etl.crm_migration_ids_20161005 id
LEFT JOIN dbo.ContactBase_20161005 cb ON id.contactid = cb.ContactId
LEFT JOIN (
	SELECT *
	FROM dbo.CustomerAddressBase_20160930
	WHERE AddressNumber = 1 AND AddressTypeCode IS null)cab ON id.contactid = cab.parentid
LEFT JOIN prodcopy.contact pc ON id.contactid = pc.contactid
WHERE cb.statecode = 0 AND cb.merged = 0 AND pc.contactid IS null
