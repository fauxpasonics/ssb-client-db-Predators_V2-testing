...
CREATE VIEW [etl].[vw_Load_DimCustomer_FanMaker] AS 

WITH FirstStep (Email, Profile_URL, Created_At)
AS (
	SELECT email, profile_Url, MAX(Created_At)
	FROM ods.Fanmaker_Users
	WHERE email_deliverable = 'True'
	GROUP BY email, profile_url
	)
...
SELECT  DB_NAME() AS [SourceDB]
      , 'FanMaker' AS [SourceSystem]
      , NULL AS [SourceSystemPriority]


/*Standard Attributes*/
      , CAST(a2.email AS NVARCHAR(100)) AS [SSID]
      ...
      , TRY_CAST((CASE WHEN a2.birthdate = '' THEN NULL
			ELSE a2.birthdate END) AS DATE) AS [Birthday]
      , a2.gender AS [Gender]
      , 0 [MergedRecordFlag]
      , NULL [MergedIntoSSID]
...
/*AddressPrimary*/
      , a2.[address] AS [AddressPrimaryStreet]
	  , NULL AS [AddressPrimarySuite]
      , a2.city AS [AddressPrimaryCity]
      , a2.[state] AS [AddressPrimaryState]
      , a2.zip AS [AddressPrimaryZip]
...
/*Email*/
      , a2.email AS [EmailPrimary]
      , NULL AS [EmailOne]
      , NULL AS [EmailTwo]

/*Extended Attributes*/
      , NULL AS [ExtAttribute1] --nvarchar(100)
      , a2.profile_url AS [ExtAttribute2]
      , a2.membership_assignment AS [ExtAttribute3]
      , a2.email_deliverable AS [ExtAttribute4]
      , a2.fanfluence AS [ExtAttribute5]
      , a2.relationship_status AS [ExtAttribute6]
      , a2.religion AS [ExtAttribute7]
      , a2.political AS [ExtAttribute8]
      , NULL AS [ExtAttribute9]
      , NULL AS [ExtAttribute10]
      , a2.points_available AS [ExtAttribute11]
      , a2.points_spent AS [ExtAttribute12]
      , a2.total_points_earned AS [ExtAttribute13]
      , a2.social_points AS [ExtAttribute14]
      , a2.ticketing_points AS [ExtAttribute15]
      , a2.ticketing_spend AS [ExtAttribute16]
      , a2.pos_points AS [ExtAttribute17]
      , a2.pos_spend AS [ExtAttribute18]
...
/*Source Created and Updated*/
      , NULL AS [SSCreatedBy]
      , NULL AS [SSUpdatedBy]
      , a2.created_at [SSCreatedDate]
      , NULL [SSUpdatedDate]
      , a2.ETL__CreatedDate AS CreatedDate
      , a2.ETL__UpdatedDate AS UpdatedDate
      , NULL [AccountId]
      , 0 IsBusiness
	  , 0 AS IsDeleted
	  , CASE WHEN gu.ticketmasterID IS NOT NULL THEN CONCAT('TM-',gu.ticketmasterID)
		ELSE NULL END AS [customer_matchkey]

--		select COUNT(*)
	FROM ods.FanMaker_Users AS a2 WITH ( NOLOCK )
	JOIN FirstStep fs
		ON a2.email = fs.email
		AND a2.profile_url = fs.profile_url
		AND a2.created_at = fs.created_at
	JOIN stg.FanMaker_GetUsers gu (NOLOCK)
		ON a2.email = gu.username
	WHERE 1 = 1
	AND a2.ETL__UpdatedDate > DATEADD(DAY,-3,GETDATE())
