...
CREATE PROCEDURE [etl].[FanMaker_LoadDimCustomer] 
...
select * 
INTO #SrcData
FROM ods.vw_FanMaker_LoadDimCustomer
...
THEN UPDATE SET 
	...
	LEFT JOIN ods.vw_FanMaker_LoadDimCustomer fmdc ON t.SSID = fmdc.ssid AND t.EmailPrimary = fmdc.EmailPrimary
	WHERE t.SourceSystem = 'FanMaker' AND fmdc.SSID IS NULL AND t.IsDeleted = 0
